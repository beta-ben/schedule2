...
export default function TaskConfigPanel({ tasks, onCreate, onUpdate, onArchive, onDelete, dark }:{
  tasks: Task[]
  ...
  <div className={["border rounded-xl overflow-auto", dark?"border-neutral-800":"border-neutral-300"].join(' ')}>
        <table className="min-w-full text-sm">
          ...
          <tbody className={dark?"divide-y divide-neutral-800":"divide-y divide-neutral-200"}>
            {active.length===0 ? (
              ...
              <tr key={t.id} className={dark?"hover:bg-neutral-900":"hover:bg-neutral-50"}>
                {editingId===t.id ? (
                  <>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
        <label className="text-sm flex flex-col md:col-span-2">
          ...
          <input className={["w-full border rounded-xl px-3 py-2", dark&&"bg-neutral-900 border-neutral-700"].filter(Boolean).join(' ')} value={draft.name} onChange={e=>setDraft({...draft, name:e.target.value})} />
        </label>
        <label className="text-sm flex flex-col">
          <span className="mb-1">Color</span>
          <input type="color" className={["w-full border rounded-xl h-10", dark&&"bg-neutral-900 border-neutral-700"].filter(Boolean).join(' ')} value={draft.color} onChange={e=>setDraft({...draft, color:e.target.value})} />
        </label>
        <div>
          <button onClick={()=>{ if(!draft.name.trim()) return; onCreate({ ...draft, name:draft.name.trim() }); setDraft({ name:'', color:'#2563eb' }) }} className={["h-10 rounded-xl border font-medium px-4", dark?"bg-neutral-800 border-neutral-700":"bg-blue-600 border-blue-600 text-white"].join(' ')}>Add Posture</button>
        </div>
      </div>
