...
// - Ensures VITE_SCHEDULE_API_BASE is set in .env.production
...
try{
  ...
  const base = kv['VITE_SCHEDULE_API_BASE']
  if(!base) fail('VITE_SCHEDULE_API_BASE not set in .env.production')
  ...
  console.log('1) Login: curl -i -X POST -H "content-type: application/json" --data "{\\"password\\":\\"<pw>\\"}" https://api.teamschedule.cc/api/login')
  console.log('   - Expect Set-Cookie for sid and csrf; SameSite=Lax; Secure; Domain=.teamschedule.cc')
  console.log('2) Read: curl -i -X GET --cookie "sid=<cookie>; csrf=<cookie>" https://api.teamschedule.cc/api/schedule')
  console.log('3) Write: curl -i -X POST -H "content-type: application/json" -H "x-csrf-token: <csrf>" --cookie "sid=<cookie>; csrf=<cookie>" --data "{\\"shifts\\":[],\\"pto\\":[],\\"updatedAt\\":\\"<iso>\\"}" https://api.teamschedule.cc/api/schedule')
